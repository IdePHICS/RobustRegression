...
@njit(error_model="numpy", fastmath=True)
def _ZoutBayes_double_noise_erm(y, xi, m, q, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (1 - eps) * np.exp(
        -((y - np.sqrt(eta) * xi) ** 2) / (2 * (1 - eta + delta_in))
    ) / np.sqrt(2 * np.pi * (1 - eta + delta_in)) + eps * np.exp(
        -((y - np.sqrt(eta) * xi) ** 2) / (2 * (1 - eta + delta_out))
    ) / np.sqrt(
        2 * np.pi * (1 - eta + delta_out)
    )


@njit(error_model="numpy", fastmath=True)
def ZoutBayes_double_noise(y, omega, V, delta_in, delta_out, eps):
    return np.exp(-((y - omega) ** 2) / (2 * (V + delta_out))) * (
        (1 - eps)
        * np.exp(
            ((y - omega) ** 2)
            * (1 / (2 * (V + delta_out)) - 1 / (2 * (V + delta_in)))
        )
        / np.sqrt(2 * np.pi * (V + delta_in))
        + eps / np.sqrt(2 * np.pi * (V + delta_out))
    )


@njit(error_model="numpy", fastmath=True)
def DZoutBayes_double_noise(y, omega, V, delta_in, delta_out, eps):
    return (y - omega) * (
        (1 - eps)
        * np.exp(-((y - omega) ** 2) / (2 * (V + delta_in)))
        / np.sqrt(2 * np.pi * (V + delta_in) ** 3)
        + eps
        * np.exp(-((y - omega) ** 2) / (2 * (V + delta_out)))
        / np.sqrt(2 * np.pi * (V + delta_out) ** 3)
    )


@njit(error_model="numpy", fastmath=True)
def _foutBayes_double_noise_erm(y, xi, m, q, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    ...
    return (
        (y - np.sqrt(eta) * xi)
        * (
            (1 - eps) * small_exponential / np.power(1 - eta + delta_in, 3 / 2)
            + eps * large_exponential / np.power(1 - eta + delta_out, 3 / 2)
        )
        / (
            (1 - eps) * small_exponential / np.power(1 - eta + delta_in, 1 / 2)
            + eps * large_exponential / np.power(1 - eta + delta_out, 1 / 2)
        )
    )


@njit(error_model="numpy", fastmath=True)
def foutBayes_double_noise(y, omega, V, delta_in, delta_out, eps):
    # small_exponential = np.exp(-((y - omega) ** 2) / (2 * (V + delta_in)))
    # large_exponential = np.exp(-((y - omega) ** 2) / (2 * (V + delta_out)))
    # return (
    #     (y - omega)
    #     * (
    #         (1 - eps) * small_exponential / np.sqrt((V + delta_in) ** 3)
    #         + eps * large_exponential / np.sqrt((V + delta_out) ** 3)
    #     )
    #     / (
    #         (1 - eps) * small_exponential / np.sqrt(V + delta_in)
    #         + eps * large_exponential / np.sqrt(V + delta_out)
    #     )
    # )
    exponential = np.exp(
        ((y - omega) ** 2) * (1 / (2 * (V + delta_out)) - 1 / (2 * (V + delta_in)))
    )
    return (
        (y - omega)
        * (
            (1 - eps) * exponential / np.sqrt((V + delta_in) ** 3)
            + eps / np.sqrt((V + delta_out) ** 3)
        )
        / (
            (1 - eps) * exponential / np.sqrt(V + delta_in)
            + eps / np.sqrt(V + delta_out)
        )
    )
...
def _ZoutBayes_decorrelated_noise_erm(
    y, xi, m, q, sigma, delta_in, delta_out, eps, beta
):
    eta = m ** 2 / q
    return (1 - eps) * np.exp(
        -((y - np.sqrt(eta) * xi) ** 2) / (2 * (1 - eta + delta_in))
        ...
        / (2 * (beta ** 2 * (1 - eta) + delta_out))
    ) / np.sqrt(
        2 * np.pi * (beta ** 2 * (1 - eta) + delta_out)
    )


@njit(error_model="numpy", fastmath=True)
def ZoutBayes_decorrelated_noise(y, omega, V, delta_in, delta_out, eps, beta):
    return (1 - eps) * np.exp(-((y - omega) ** 2) / (2 * (V + delta_in))) / np.sqrt(
        2 * np.pi * (V + delta_in)
    ) + eps * np.exp(
...
@njit(error_model="numpy", fastmath=True)
def DZoutBayes_decorrelated_noise(y, omega, V, delta_in, delta_out, eps, beta):
    small_exponential = np.exp(-((y - omega) ** 2) / (2 * (V + delta_in))) / np.sqrt(
        2 * np.pi
    )
    large_exponential = np.exp(
        -((y - beta * omega) ** 2) / (2 * (beta ** 2 * V + delta_out))
...
def _foutBayes_decorrelated_noise_erm(
    ...
    return (
        ...
        * (1 - eps)
        * small_exponential
        / np.power(1 - eta + delta_in, 3 / 2)
        + eps
        * beta
        * (y - beta * np.sqrt(eta) * xi)
        * large_exponential
        / np.power(beta ** 2 * (1 - eta) + delta_out, 3 / 2)
    ) / (
        (1 - eps) * small_exponential / np.power(1 - eta + delta_in, 1 / 2)
        + eps * large_exponential / np.power(beta ** 2 * (1 - eta) + delta_out, 1 / 2)
    )


@njit(error_model="numpy", fastmath=True)
def foutBayes_decorrelated_noise(y, omega, V, delta_in, delta_out, eps, beta):
    small_exponential = np.exp(-((y - omega) ** 2) / (2 * (V + delta_in)))
    large_exponential = np.exp(
        -((y - beta * omega) ** 2) / (2 * (beta ** 2 * V + delta_out))
    )
    return (
        (y - omega) * (1 - eps) * small_exponential / np.power(V + delta_in, 3 / 2)
        + eps
        * beta
        * (y - beta * omega)
        * large_exponential
        ...
        (1 - eps) * small_exponential / np.power(V + delta_in, 1 / 2)
        + eps * large_exponential / np.power(beta ** 2 * V + delta_out, 1 / 2)
    )
    # exponential = np.exp(
    #     ((y - omega) ** 2) / (2 * (V + delta_in))
    #     - ((y - beta * omega) ** 2) / (2 * ((beta ** 2) * V + delta_out))
    # )
    # return (
    #     (y - omega)
    #     * (
    #         (1 - eps) / np.sqrt((V + delta_in) ** 3)
    #         + eps * exponential / np.sqrt(((beta ** 2) * V + delta_out) ** 3)
    #     )
    #     / (
    #         (1 - eps) / np.sqrt(V + delta_in)
    #         + eps * exponential / np.sqrt((beta ** 2) * V + delta_out)
    #     )
    # )
...
@njit(error_model="numpy", fastmath=True)
def q_integral_BO_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
        * ZoutBayes_double_noise(y, np.sqrt(q) * xi, 1 - q, delta_in, delta_out, eps)
        * (
            foutBayes_double_noise(
                y, np.sqrt(q) * xi, 1 - q, delta_in, delta_out, eps
            )
            ** 2
        )
    )
...
@njit(error_model="numpy", fastmath=True)
def m_integral_L2_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
        * ZoutBayes_double_noise(
            y, np.sqrt(eta) * xi, (1 - eta), delta_in, delta_out, eps
        )
        * foutBayes_double_noise(
            y, np.sqrt(eta) * xi, (1 - eta), delta_in, delta_out, eps
        )
...
@njit(error_model="numpy", fastmath=True)
def q_integral_L2_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def sigma_integral_L2_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def m_integral_L1_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def q_integral_L1_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def sigma_integral_L1_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def m_integral_Huber_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps, a):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
@njit(error_model="numpy", fastmath=True)
def q_integral_Huber_double_noise(y, xi, q, m, sigma, delta_in, delta_out, eps, a):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
...
def sigma_integral_Huber_double_noise(
    y, xi, q, m, sigma, delta_in, delta_out, eps, a
):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
...
def q_integral_BO_decorrelated_noise(
    y, xi, q, m, sigma, delta_in, delta_out, eps, beta
):
    return (
        np.exp(-(xi ** 2) / 2)
        / np.sqrt(2 * np.pi)
        * ZoutBayes_decorrelated_noise(
            y, np.sqrt(q) * xi, 1 - q, delta_in, delta_out, eps, beta
        )
        * (
...
def m_integral_L2_decorrelated_noise(
    ...
    return (
        ...
        / np.sqrt(2 * np.pi)
        * ZoutBayes_decorrelated_noise(
            y, np.sqrt(eta) * xi, (1 - eta), delta_in, delta_out, eps, beta
        )
        * foutBayes_decorrelated_noise(
            y, np.sqrt(eta) * xi, (1 - eta), delta_in, delta_out, eps, beta
        )
...
def m_integral_Huber_decorrelated_noise(
    y, xi, q, m, sigma, delta_in, delta_out, eps, beta, a
):
    eta = m ** 2 / q
    return (
        np.exp(-(xi ** 2) / 2)
...
def q_hat_equation_BO_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
        lambda y, xi: q_integral_BO_double_noise(
            y, xi, q, m, sigma, delta_in, delta_out, eps
        ),
    ...
    for xi_funs, y_funs in zip(domain_xi, domain_y):
        integral_value += dblquad(
            ...
            y_funs[0],
            y_funs[1],
            args=(q, m, sigma, delta_in, delta_out, eps),
            epsabs=EPSABS,
            epsrel=EPSREL,
        )[0]
...
def m_hat_equation_L2_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
...
def q_hat_equation_L2_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
...
def sigma_hat_equation_L2_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
...
def m_hat_equation_L1_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
...
def q_hat_equation_L1_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
...
def sigma_hat_equation_L1_double_noise(m, q, sigma, delta_in, delta_out, eps):
    borders = find_integration_borders_square(
        lambda y, xi: q_integral_L1_double_noise(
            y, xi, q, m, sigma, delta_in, delta_out, eps
        ),
...
def m_hat_equation_Huber_double_noise(m, q, sigma, delta_in, delta_out, eps, a):
    borders = find_integration_borders_square(
    ...
    for xi_funs, y_funs in zip(domain_xi, domain_y):
        integral_value += dblquad(
            ...
            y_funs[0],
            y_funs[1],
            args=(q, m, sigma, delta_in, delta_out, eps, a),
            epsabs=EPSABS,
            epsrel=EPSREL,
        )[0]
...
def q_hat_equation_Huber_double_noise(m, q, sigma, delta_in, delta_out, eps, a):
    borders = find_integration_borders_square(
...
def sigma_hat_equation_Huber_double_noise(m, q, sigma, delta_in, delta_out, eps, a):
    borders = find_integration_borders_square(
...
def q_hat_equation_BO_decorrelated_noise(
    m, q, sigma, delta_in, delta_out, eps, beta
):
    borders = find_integration_borders_square(
    ...
    # print(borders[0][0], m,q,sigma, " - ", delta_in, delta_out, eps, beta)
    ...
    for xi_funs, y_funs in zip(domain_xi, domain_y):
        integral_value += dblquad(
            ...
            y_funs[0],
            y_funs[1],
            args=(q, m, sigma, delta_in, delta_out, eps, beta),
        )[0]
...
def m_hat_equation_Huber_decorrelated_noise(
    m, q, sigma, delta_in, delta_out, eps, beta, a
):
    borders = find_integration_borders_square(
    ...
    for xi_funs, y_funs in zip(domain_xi, domain_y):
        integral_value += dblquad(
            ...
            y_funs[0],
            y_funs[1],
            args=(q, m, sigma, delta_in, delta_out, eps, beta, a),
        )[0]
