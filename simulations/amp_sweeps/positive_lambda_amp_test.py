...
for idx, reg_param in enumerate(reg_params):
    ...
    alphas_Hub[idx], (gen_error_Hub[idx], train_error_Hub[idx], ms_Hub[idx], qs_Hub[idx], sigmas_Hub[idx]) = sweep_alpha_fixed_point(
        var_func_L2,
        var_hat_func_Huber_decorrelated_noise,
        0.1,
        30,
        ...
        initial_cond_fpe=(m_init, q_init, sigma_init),
        funs=[gen_error, training_error_huber_loss, m_order_param, q_order_param, sigma_order_param],
...
for idx in range(len(reg_params)):
    ...
    plt.plot(
        alphas_l1[idx],
        stability_l1_l2(
            ms_l1[idx], qs_l1[idx], sigmas_l1[idx], alphas_l1[idx], reg_param, delta_in, delta_out, percentage, beta
        ),
        color=color,
